...
listaDiaria = SimpleVocabulary.fromValues(['Sim','Nao'])
...
class Ievento(form.Schema, IImageScaleTraversable):
    ...
    outroespaco=schema.TextLine(title=u"Outro Espaço",description=u'Caso outro espaço, informe qual:',required=False)
    ...
    form.fieldset('detalhes',label=u"Detalhes do evento", fields=['description','observacoes'] )
    form.widget('description', WysiwygFieldWidget)
    form.widget('observacoes', WysiwygFieldWidget)
    ...
    observacoes=schema.Text(title=u"Observações do evento",description=u"Informe mais sobre combinados para esse evento",required=False)		
